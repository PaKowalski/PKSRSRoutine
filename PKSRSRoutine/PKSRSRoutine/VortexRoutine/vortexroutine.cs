...
namespace VortexRoutine
{
	/// <summary> </summary>
	public class VortexRoutine : IRoutine
	{
		...
		private void RegisterExposedSettings()
		{
			...
			PropertyInfo[] properties = typeof (VortexRoutineSettings).GetProperties(BindingFlags.Public | BindingFlags.Instance);
			foreach (PropertyInfo p in properties)
			{
				...
				{
					p.SetValue(VortexRoutineSettings.Instance, param[0]);
					return null;
				});

				_exposedSettings.Add("Get" + p.Name, param =>
				{
					return p.GetValue(VortexRoutineSettings.Instance);
				});
			}
		}
		...
		{
			var tokens = VortexRoutineSettings.Instance.BlacklistedSkillIds.Split(new[]
			{
				' ', ',', ';', '-'
		...
		public void Start()
		{
			Log.DebugFormat("[VortexRoutine] Start");

			_needsUpdate = true;

			if (VortexRoutineSettings.Instance.SingleTargetMeleeSlot == -1 &&
				VortexRoutineSettings.Instance.SingleTargetRangedSlot == -1 &&
				VortexRoutineSettings.Instance.AoeMeleeSlot == -1 &&
				VortexRoutineSettings.Instance.AoeRangedSlot == -1 &&
				VortexRoutineSettings.Instance.FallbackSlot == -1
				)
			{
				Log.ErrorFormat(
					"[Start] Please configure the VortexRoutine settings (Settings -> VortexRoutine) before starting!");
				BotManager.Stop();
			}
		}
		...
		public void Stop()
		{
			Log.DebugFormat("[VortexRoutine] Stop");
		}
						...
						FileMode.Open))
				{
					...
					if (
						!Wpf.SetupCheckBoxBinding(root, "SkipShrinesCheckBox", "SkipShrines",
							BindingMode.TwoWay, VortexRoutineSettings.Instance))
					{
						Log.DebugFormat(
					...
					if (
						!Wpf.SetupComboBoxItemsBinding(root, "SingleTargetMeleeSlotComboBox", "AllSkillSlots",
							BindingMode.OneWay, VortexRoutineSettings.Instance))
					{
						Log.DebugFormat(
					...
					if (
						!Wpf.SetupComboBoxSelectedItemBinding(root, "SingleTargetMeleeSlotComboBox",
							"SingleTargetMeleeSlot", BindingMode.TwoWay, VortexRoutineSettings.Instance))
					{
						Log.DebugFormat(
					...
					if (
						!Wpf.SetupComboBoxSelectedItemBinding(root, "SingleTargetRangedSlotComboBox",
							"SingleTargetRangedSlot", BindingMode.TwoWay, VortexRoutineSettings.Instance))
					{
						Log.DebugFormat(
					...
					if (
						!Wpf.SetupComboBoxSelectedItemBinding(root, "AoeMeleeSlotComboBox",
							"AoeMeleeSlot", BindingMode.TwoWay, VortexRoutineSettings.Instance))
					{
						Log.DebugFormat(
					...
					if (
						!Wpf.SetupComboBoxSelectedItemBinding(root, "AoeRangedSlotComboBox",
							"AoeRangedSlot", BindingMode.TwoWay, VortexRoutineSettings.Instance))
					{
						Log.DebugFormat(
					...
					if (
						!Wpf.SetupComboBoxSelectedItemBinding(root, "FallbackSlotComboBox",
							"FallbackSlot", BindingMode.TwoWay, VortexRoutineSettings.Instance))
					{
						Log.DebugFormat(
		...
		public JsonSettings Settings
		{
			get
			{
				return VortexRoutineSettings.Instance;
			}
		}
			...
			if (type == "combat")
			{
				...
				if (_flameblastSlot != -1)
				{
					if (_castingFlameblast)
					{
						...
						if (c >= VortexRoutineSettings.Instance.MaxFlameBlastCharges)
						{
