...
{
    /// <summary>Settings for the VortexRoutine. </summary>
    public class VortexRoutineSettings : JsonSettings
    {
        private static VortexRoutineSettings _instance;

        /// <summary>The current instance for this class. </summary>
        public static VortexRoutineSettings Instance
        {
            get { return _instance ?? (_instance = new VortexRoutineSettings()); }
        }

        /// <summary>The default ctor. Will use the settings path "VortexRoutine".</summary>
        public VortexRoutineSettings()
            : base(GetSettingsFilePath(Configuration.Instance.Name, string.Format("{0}.json", "VortexRoutine")))
        {
        }
        ...
        [DefaultValue(5)]
        public int BladeVortexCount
        {
            get { return _bladeVortexCount; }
            set
            {
                if(value.Equals(_bladeVortexCount))
                {
                    return;
                }
                _bladeVortexCount = value;
                NotifyPropertyChanged(() => BladeVortexCount);
            }
        }
